...
void fused_adam_cuda(at::Tensor & p, at::Tensor & p_copy, at::Tensor & m, at::Tensor & v, at::Tensor & g, at::Tensor & delta, float lr, float beta1, float beta2, float eps, float grad_scale, int step, int mode, int bias_correction, float decay);
...
// C++ interface
void adam(at::Tensor & p, at::Tensor & p_copy, at::Tensor & m, at::Tensor & v, at::Tensor & g, at::Tensor & delta, float lr, float beta1, float beta2, float eps, float grad_scale, int step, int mode, int bias_correction, float decay) {
        CHECK_INPUT(p);
        ...
        AT_ASSERTM(delta.numel() == num_elem, "number of elements in delta and p tensors should be equal");
        ...
        fused_adam_cuda(p, p_copy, m, v, g, delta, lr, beta1, beta2, eps, grad_scale, step, mode, bias_correction, decay);
}
