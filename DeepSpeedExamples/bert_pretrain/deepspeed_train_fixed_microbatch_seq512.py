...
def train(args, index, model, optimizer, finetune=False):
    ...
    for step, dataset_type in enumerate(tqdm(dataset_picker, smoothing=1,disable=True)):
        try:
            ...
            # if dist.get_rank()==0:
            #     print(" "+str(current_data_sample_count)+" ",end="")
...
def update_learning_rate_by_number_of_samples(args, config, current_data_sample_count, optimizer,current_global_step=None):
    if args.lr_schedule == "EE":
        ...
        if not args.nowarmup:
            # print(f"current data sample count is {current_data_sample_count}, checkpoint sample count is {last_global_samples_from_restore}, thus the data sample count since checkpoint load is {current_data_sample_count-last_global_samples_from_restore}, corresponding to step {(current_data_sample_count-last_global_samples_from_restore)/(32*1024)}.")
            lr_this_step = config["training"][
                "learning_rate"] * warmup_exp_decay_exp(
                    (current_data_sample_count-last_global_samples_from_restore)/(32*1024), config["training"]["decay_rate"],
                    config["training"]["decay_step"],
                    config["training"]["total_training_steps"],
                    config["training"]["warmup_proportion"])
            lr_this_step += args.lr_offset
...
def load_checkpoint(args, model):
    ...
    global global_data_samples
    global last_global_step_from_restore
    global last_global_samples_from_restore
    ...
    if args.rewarmup:
        ...
        last_global_samples_from_restore = global_data_samples
...
def main():
    ...
    start_epoch = 0
    print("args.load_training_checkpoint",args.load_training_checkpoint)
    print("args.load_checkpoint_id",args.load_checkpoint_id)
    if not None in [args.load_training_checkpoint, args.load_checkpoint_id]:
        default_batch_size=model.network.train_batch_size()
        default_micro_batch_size=model.network.train_micro_batch_size_per_gpu()
        default_gradient_accumulation_steps=model.network.gradient_accumulation_steps()
        print("train_batch_size before loading the checkpoint:",model.network.train_batch_size())
        default_lr=[]
        for pg in model.network.optimizer.optimizer.param_groups:
            default_lr.append(float(pg["lr"]))
        start_epoch = load_checkpoint(args, model)
        # for var_name in optimizer.state_dict():
        #     print(var_name, "\t", optimizer.state_dict()[var_name])
        print("model.network.dp_world_size right after checkpoint loading",model.network.dp_world_size)
        print("model.network.train_micro_batch_size_per_gpu() right after checkpoint loading",model.network.train_micro_batch_size_per_gpu())
        model.network.dp_world_size=torch.distributed.get_world_size()
        if args.similarity_target is None:
            model.network.adaptive_batch_params=defaultdict(lambda: None)
            model.network._config.train_batch_size=default_batch_size
            model.network._config.train_micro_batch_size_per_gpu=default_micro_batch_size
            model.network._config.gradient_accumulation_steps=default_gradient_accumulation_steps
            model.network.gradient_accumulation_offset=0
            model.network.gradient_accumulation_start=0
            if hasattr(model.network.optimizer,"optimizer"):
                for lr,pg in zip(default_lr,model.network.optimizer.optimizer.param_groups):
                    pg["lr"]=lr
            else:
                raise NotImplementedError
        print("model.network.dp_world_size after reset",model.network.dp_world_size)
        print("Current lr:",model.network.optimizer.optimizer.param_groups[0]["lr"])
        print("Current train_batch_size:",model.network.train_batch_size())
        print("Current train_micro_batch_size_per_gpu:",model.network.train_micro_batch_size_per_gpu())
        print("Current gradient_accumulation_steps:",model.network.gradient_accumulation_steps(),flush=True)
#qy
    if(args.start_accumulation_steps):
        model.network.change_gradient_accumulation_steps(args.start_accumulation_steps,smooth=False)
        print("Gradient accumulation steps changed to ",model.network.gradient_accumulation_steps())
        print("Lr modifier changed to ",model.network.adaptive_batch_params["global_lr_modifier"])
    if(args.start_adjust_step):
        model.network.gradient_step_size=args.start_adjust_step
        print("start_adjust_step changed to ",model.network.gradient_step_size)
