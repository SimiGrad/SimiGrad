...
def train(args, index, model, optimizer, finetune=False):
    ...
    if args.config['training']['async_worker']:
        worker = AsyncWorker(dataloaders, dataset_picker,max_queue_length=256)
        worker.start()

    epoch_step = 0
    step=0
    # for step, dataset_type in enumerate(tqdm(dataset_picker, smoothing=1,disable=True)):
    while step<len(dataset_picker):
        step+=1
        try:
            batch=[]
            while step<len(dataset_picker) and len(batch)<model.network.train_micro_batch_size_per_gpu():
                step+=1
                if args.config['training']['async_worker']:
                    batch.append(worker.get())
                else:
                    batch.append(next(dataloaders[dataset_type]))
            batch=tuple(torch.cat(t) for t in zip(*batch))
            # for t in batch:
            #     print(t.shape)
            # raise Exception
...
def prepare_model_optimizer(args):
    ...
    model.network, optimizer, _, _ = deepspeed.initialize(
        ...
        adaptive_batch_params={"enable_adjust": True, "verbose": False, "similarity_target":0.75,"batch_size_upper_bound":65536*2,"max_micro_batch_size":256})
